...
import { ArrowLeftIcon, SpeakerIcon, BookmarkIcon, PuzzleIcon } from './Icons';
...
const RelatedWordsModal: React.FC<RelatedWordsModalProps> = ({ 
  ...
  const renderWordItem = (word: RelatedWord) => (
    <div 
        key={word.english}
        className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50 transition hover:bg-slate-800"
    >
        ...
        <div className="flex items-start gap-2 mt-2">
            <span className="px-2 py-0.5 text-[10px] rounded uppercase tracking-wider font-bold whitespace-nowrap mt-0.5 bg-indigo-500/20 text-indigo-300">
                {word.type}
            </span>
  ...
  return (
    <div className="h-full w-full bg-slate-900 flex flex-col pt-6 pb-24 overflow-hidden relative z-50">
      ...
      <div className="px-6 mb-4 flex items-center justify-between">
        ...
        <h2 className="text-xl font-bold font-thai text-white">คลังคำศัพท์ที่เกี่ยวข้อง</h2>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-6">
        ...
        {/* Original Word Card */}
        <div className="bg-slate-800 rounded-3xl p-6 shadow-xl flex items-center justify-between border border-slate-700">
            <div>
                <p className="text-xs text-slate-400 uppercase font-bold mb-1">Target Word</p>
        ...
        {/* Section: Related Vocabulary (Verbs removed) */}
        <div>
            <div className="flex items-center gap-2 mb-3">
                <span className="w-1 h-6 bg-indigo-500 rounded-full"></span>
                <h3 className="text-lg font-bold text-white font-thai">ศัพท์ใกล้เคียง (Related Vocabulary)</h3>
            </div>
